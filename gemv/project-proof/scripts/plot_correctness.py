...
ax.axis("off")
ax.text(0.5, 0.90, "CUDA GEMV Correctness", ha="center", va="center", fontsize=14, weight="bold")
ax.text(0.5, 0.80, f"Pass={sum(flags)}/{len(flags)} | MaxDiff={max(diffs):.2e}", ha="center", va="center", fontsize=10)
table = ax.table(
    cellText=[[v, f"{d:.2e}", "PASS" if ok else "FAIL"] for v, d, ok in zip(labels, diffs, flags)],
