#!/usr/bin/env bash
# 按 GEMV 版本分别采集完整 NCU Section（与 reduce / scripts/ncu_metrics.inc.sh 一致）。
# 用法（在 gemv 目录已编译 build/gemv_bench 后）：
#   bash project-proof/scripts/profile_ncu.sh
# 环境变量：
#   BENCH_ITERS    默认 1
#   RUN_NCU_CSV    若设为 1，额外导出 gemv_ncu.csv（全版本一次跑）
set -euo pipefail
...
BIN_PATH="$ROOT_DIR/build/gemv_bench"
OUT_DIR="$ROOT_DIR/project-proof/profiling/ncu"
CSV_BASE="$OUT_DIR/gemv_ncu"
...
run_ncu_rep() {
  local tag="$1"
  local kernel="$2"
  local out_prefix="$OUT_DIR/gemv_${tag}_profile"
  local err
  err="$(mktemp)"
  set +e
  ncu \
    ...
    "${section_args[@]}" \
    -o "$out_prefix" \
    env BENCH_ITERS="$BENCH_ITERS" GEMV_PROFILE_ONLY="$tag" "$BIN_PATH" 2>"$err"
  local status=$?
  set -e
...
for entry in "${PROFILE_TARGETS[@]}"; do
  tag="${entry%%:*}"
  kernel="${entry#*:}"
  echo "== Profiling ${tag} (${kernel}) -> gemv_${tag}_profile.ncu-rep"
...
echo "Done. Reports under: ${OUT_DIR}/gemv_*_profile.ncu-rep"
