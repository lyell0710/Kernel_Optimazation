...
| **reduce v7 跨工具链行为不一致** | **待主力机确认；确认前不得据此改 EXP-K04** | 同一 bench 同一 67.1 MB 下 v7 = **1.600 ms**（本机 CUDA 12.8）对 **0.029824 ms**（主力机 13.2），差 **54×**，而 baseline/v5/cuBLAS/CUB 四项吻合（≤3.5%）。已排除编译目标、迭代次数、同跑干扰；NCU 实测 kernel 仅 76 μs，差额在 two-pass 的 launch/同步。本机 1.60 ms 与 Laptop 时代 1.66 ms 吻合。**影响**：EXP-K04「L2 区间 CUB 快 33.3%」以 v7 为对照，在 12.8 下变成快 81 倍。EXP-K09 §5.14/§6.14 |
...
- 2026-08-26 EXP-K05《LLM 融合逐元素算子三件套》新增待办 —— **①② 已闭环，仅剩 ③**：
  ...
  ~~②rope v2 在 HBM 区间比 v1 慢 1.1% 未剖~~ → **已由 nsys 解决**：v1 对 q/k 各调一次 = 248 次 launch，v2 合并为 124 次（`docs/ncu_reading_guide.md` §4 第 4 条）。
