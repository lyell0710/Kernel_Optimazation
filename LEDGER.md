...
| [EXP-K06](records/EXP-K06_w8a8_linear.md) | W8A8 linear 完整链路：per-token 量化 + INT8 GEMM/GEMV + 融合反量化 | w8a8_linear | 2026-08-26 | 完成 | prefill **2.167x** bf16 cuBLAS；decode 的 M=1 库路径不可用，自写 dp4a GEMV **1.972x**；多一次 `.contiguous()` 则整链路跌到 **0.726x**（3.6 倍单步差距全来自 stride）；quant 仅占链路 **1.7%** → `w8a8/project-proof/data/derived_w8a8_vec-after_stability.csv` |
...
| [EXP-K10](records/EXP-K10_gemm_fa2_v5_mma_ptx.md) | gemm/fa2 v5 = mma PTX + ldmatrix + smem padding | gemm_fa2_v5_mma_ptx | 2026-08-30 | 完成 | gemm v5 正确性 PASS、**135.1±2.1 TFLOPS = cuBLAS 88.1%**（v4 132.8=85.6%）；fa2 v5 **35.0 TFLOPS**（v4 34.8，+0.6%）；**负结果**：ldmatrix 消 bank conflict 后两仓性能均持平，smem 冲突不是瓶颈 → `project-proof/data/derived_{gemm4096,fa2}_v5_stability.csv` |
...
| GEMM「超过/追平 cuBLAS」 | 禁用（现状 **85.6%**，**该数字对工具链敏感**；v5 的 88.1% 不构成新基线，见右） | **权威口径仍是 v4 的 85.6%**（132.8±0.49 TFLOPS，已拍板见下节）。v5 测得 135.1±2.08 TFLOPS（88.1%），但差值 +2.3 TFLOPS 对合并 σ≈2.14 仅 **1.1 倍**，3 轮内**不可分辨**；且 v5 的 std 由 0.49 涨到 2.08（方差翻四倍、均值未动）。按「差值须超轮间 std」的既有判据，v5 不构成新基线，EXP-K10 的「持平」结论与此一致。原注：v4 同口径 85.6%（EXP-K02 §8）；采集主机 CUDA 12.8 同协议测得 **77.9%**（EXP-K07 §5.2/§6.2）。**权威工具链已拍板**：88.1%（CUDA 13.2，gemm v5）为权威口径，85.6%（v4）为上一实现、77.9%（CUDA 12.8）降为跨工具链参照、不得单独引用，出现 88.1% 处加括注「CUDA 13.2；v4 85.6%、12.8 下 77.9%」。注：v5 与 v4 差 2.5 点但 std 2.08 未达显著，报「v5 快」须带完整限定。Triton 版数字不得挪用 |
| FA2「达到 sdpa/Triton 水平」 | 禁用（现状 28%，**v5 未解锁**） | EXP-K10：fa2 v5（mma PTX + ldmatrix）性能持平 v4（35.0 vs 34.8 TFLOPS），仍是 Triton 28% 量级——ldmatrix 不是瓶颈解法，该红线维持禁用 |
...
|「swizzle 能消除该瓶颈」 | **gemm 与 fa2 均证伪（端到端口径）** | EXP-K10：两仓 v5 用 ldmatrix/padding 消 bank conflict 后性能均持平（gemm +1.7% 未达显著、fa2 +0.6%）→ **smem 冲突波前占比高 ≠ 性能瓶颈**（smem 带宽有富余、冲突被其他 stall 掩盖），swizzle 不是解法；计数器口径（bank_conflicts/short_scoreboard 是否真下降）需采集机复验 |
...
- **两处对外数字勘误（2026-08-29，采集主机核对）**：
  ① `activation` 融合一级 **1.675x → 1.678x**（EXP-K05 当轮、主力机 CUDA 13.2 口径）。当轮指针文件
  `derived_activation_stability.csv` 的 `hbm,v1` 三口径一致给 1.678（`speedup_vs_v0_mean`=1.67833、
  延迟比 1.11674/0.665349=1.6784、带宽比 907.733/540.867=1.6783）。
  源头是 `records/EXP-K05` 的笔误，已扩散 10 处，本轮统一订正。
  **现行对外口径为 1.680x**，取自权威 `activation/project-proof/data/derived_activation_vec-after_stability.csv`
  的 `hbm,v1`，三口径同样一致（`speedup_vs_v0_mean`=1.67967、延迟比 1.11762/0.665327=1.6798、
  带宽比 907.8/540.433=1.6798）。结论方向不变（与 5/3=1.667 的吻合度 0.48% → 0.66% → 0.78%，同量级）。
...
- ~~待同 harness 复测：一切 vs Triton/sdpa 数字~~ —— **GEMM/FA2 已完成**（`scripts/same_harness/`,同进程同数据同协议）:FA2 cuda_v4/triton = **28.1%**,与跨 harness 报的 28% 吻合到小数点后一位;新增 cuda_v4/sdpa = **23.4%**。EXP-K05 的三个融合逐元素算子本就同 harness。红线已由推断级升为实测级。
...
- ~~采集主机任务书（第二轮）~~ —— **已完成并归档**：`PROFILING_HOST_TASKS.md` 按其自身 §7 第 4 条退场，原文移至 `docs/archive/2026-08-29_profiling_host_tasks.md`（`records/EXP-K07`、`records/EXP-K09` 按旧路径引用它，史料不改）。本轮结论入 EXP-K09《向量化修复后的扇区账复采》；仓库根目录自此只有本文件一个状态源。
- 开放问题（EXP-K07《NCU 计数器闭环》 §7.4 第 1 条；EXP-K09 §7 记为本轮未做）：**主力机的 cuBLAS 选了哪个 GEMM kernel 未查**。采集主机（CUDA 12.8）选的是 `ampere_fp16_s1688gemm_fp16_128x128`、grid `(32,32,3)`（z=3 即 split-K），若 CUDA 13.2 选了不同实现，红线表「GEMM 85.6% vs 77.9%」那条的工具链归因即彻底闭合；`nsys profile --trace=cuda` 拿 kernel 名即可，不需要计数器权限。
...
- ~~待修~~ **已完成（五处，不含 dequant）**（`bc18547`）：`rope` v3/v4、`activation` v2/v3、`w8a8` quant.v2 五处 `BF16x8` 载体已替换为 union 定义，读侧 SASS 判据转正（`LDG.E.128` rope v3 0→4、v4 2→4，activation v2/v3 0→2，w8a8 quant.v2 0→2；逐条见 EXP-K08《BF16x8 向量化未兑现的定位与修复》 §5.2 兑现表）。
  - **未纳入**：`w8a8/src/dequant.cu` 一行未改（`bc18547` 的改动清单只含上述五个 `.cu`）。读侧本就兑现——`I32x4 { int32_t v[4]; }` 的成员是原生 4 字节整型，编得出 `LDG.E.128`；写侧 `struct alignas(8) BF16x4 { __nv_bfloat162 h[2]; };`（`w8a8/src/dequant.cu:46`）仍被拆成 2 条 32 位 `STG.E`，没有 `STG.E.64`，**该处写侧判据未转正**（`docs/sass_evidence_ladder.md` §6.1；修法同理换 `union{float2 raw;}`）。
- 开放项（由上条派生，不阻塞）：`w8a8` `dequant` 的写侧判据要不要转正**先按红线表末行的浪费比判据评估再动手**，不能只凭 SASS 判据——同一算子的 quant.v2 就是兑现后收益 ≈0 的反例（EXP-K08 §5.2）；写侧计数器数据在 `w8a8/project-proof/profiling/ncu/w8a8_dequant_v1_profile.ncu-rep`。
