...
| v1 | 一线程一对，读 2 写 2 | 784.7±1.5 GB/s (77.9%) | 2025.6 GB/s | 11.22 us |
...
| torch.compile | — | 877.5±0.2 GB/s (87.1%) | 562.8 GB/s | 81.55 us |
...
**同一个改动在两个区间收益差 32 倍。** v1→v2（q/k 合并 launch）在 HBM 区间是 -1.2%（噪声内），在 decode 区间是 1.39x。带宽饱和时省一次 launch 毫无意义； T=1 时一次 launch 就能主导总时间。优化必须绑定工作区间来谈。
