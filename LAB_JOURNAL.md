...
- **关键数字**:**reduce v6/v7 排序反转**(Laptop 回退版 → 4090 最优且反超
  ...
  系 CPU 口径,PyTorch-eager 4090 对照移 triton-kernels#EXP-T03《三件套移植 + torch 绑定》。
...
- **影响与处置**:"softmax 快 cuBLAS 26%/34%"全链作废(EXP-K01《四 kernel 4090 重基准》表格、
...
- **做了什么**:新建 gemm/ 子项目,CUDA 原生 v0 naive → v1 smem tile →
  ...
  4096³ 三轮 + ptxas 资源画像;EXP-K02《CUDA Tensor Core GEMM 版本梯》八节记录。
...
- **做了什么**:flash-attn/ 子项目,v0 warp-per-row 在线 softmax → v1 smem
  ...
  S/P;协议对齐 triton-kernels test_fa2.py,3 轮 + ptxas;EXP-K03《CUDA FA2 forward 简化版版本梯》。
