...
> **更正指引（原句为史料，保持原样；下列为现行口径）**：本记录 reduce 侧的对照口径不对称（v7 侧计时含分配器开销、CUB 侧不含）。同协议重测后，L2 常驻区间 CUB 快 **12.1%**（不是 33.3%），HBM-bound 区间两者在测量分辨率内**贴平**（不是「差 0.7%」，v7 反快 0.1%）。详见 §7.1 更正块。
...
### 7.1 更正块：reduce 对照口径不对称(追加,§4/§5/§6 原文不动)

本记录 reduce 两区间的对照口径不对称：v1–v7 每次调用都在**计时区内** `cudaMalloc` + `cudaFree`（v6/v7 还多一次 `cudaGetDeviceProperties`），而 CUB / cuBLAS / baseline 侧都没有。偏置只加在手写侧，且版本越快被罚得越重——对 v7（约 30 μs）占四分之一，对 v1（约 118 μs）只占 6%，于是版本梯越往后与 CUB 的差距被系统性高估。

把分配与设备属性提到计时外（与 CUB 的 temp_storage 同款）后，同协议、同机（主力机 RTX 4090 / CUDA 13.2）3 轮重测：

| 区间 | 项 | 本记录（口径不对称） | 同口径重测（现行权威） |
|---|---|---:|---:|
| L2 常驻 67.1 MB | CUB | 0.019828±0.000098 ms | 0.019808±0.000136 ms |
| L2 常驻 67.1 MB | v7 | 0.029740±0.000124 ms | 0.022535±0.000065 ms |
| L2 常驻 67.1 MB | 判定 | CUB 快 **33.3%** | CUB 快 **12.1%** |
| HBM-bound 1.07 GB | v7 | 1.13483 ms / 946.2 GB/s / 93.9% | 1.12659±0.00033 ms / 953.1 GB/s / 94.5% |
| HBM-bound 1.07 GB | CUB | 1.12730 ms / 952.5 GB/s / 94.5% | 1.12789±0.00041 ms / 952.0 GB/s / 94.4% |
| HBM-bound 1.07 GB | 判定 | CUB 快 **0.7%** | **测量分辨率内贴平**（v7 反快 0.1%，与轮间 std 同量级） |

两处读法变化：

- **L2 区间：结论方向不变，幅度变小。** CUB 仍然快，§5 之 2「L2 常驻区间才是厂商库拉开差距的地方」这个论点成立；只是幅度是 12.1% 而不是 33.3%——33.3 个百分点里约 20 个来自计时区内的分配器开销，不是算法差距。
- **HBM 区间：连方向都不成立了。** 同口径下 v7 反快 0.1%，正确写法是「在测量分辨率内贴平」，不得写成任何一方领先。§4.1 与 §5 之 1 的「差 0.7%」、以及 §7「新增可用口径」第一条的 93.9% / 0.7%，按本块理解：手写 reduce 达理论峰值 **94.5%**，与 CUB **贴平**；第二条的 33.3% 按 **12.1%** 理解。softmax 与 gemv 两条不受影响（它们的对照本就不含这项偏置）。

数据：`records/data/exp_k04_cuda_reduce_calfix_3rounds.csv`（L2 常驻）、`records/data/exp_k04_reduce_hbmbound_calfix_3rounds.csv`（HBM-bound）；原始 `cuda-reduce/project-proof/data/*_{l2,hbm}_*_calfix_r{1,2,3}.csv`，脚本 `scripts/reduce_regime_rebench.sh`。口径不对称的定位与量化过程见 EXP-K09 §6.20。
