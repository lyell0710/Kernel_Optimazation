...
> **一句话结论**：在计数器权限打开的 4090 虚机上补齐 18 份报告，把 `ncu_reading_guide` §4 中**三条**长期挂着"推断"的命题（第 1、2、5 条）转成实测——GEMM v4 与 cuBLAS 的性能差距（77.9%）与两者 Tensor 管线利用率之比（77.7%）在 0.2 个百分点内重合；fused-norm 的"第二次读被缓存接住"由 DRAM 读扇区恒为 2.000×S 直接证实；FA2 的 `short_scoreboard`（smem）stall 在 v4 上达 50.13%、而全局访存 stall 已降至 0.31%，坐实"smem 往返是剩余瓶颈"；
...
| `asum_kernel`（真 cuBLAS） | (216,1,1) | 9.42 | 94.64 | **98.99** | 0.10 | 94.42 | 0.66 |
...
v7（DRAM 96.35%、long_sb 96.86%、SM 仅 8.64%）与真 cuBLAS `asum_kernel`（DRAM 94.64%、long_sb 94.42%）
**画像几乎重合**——两者撞在同一堵墙上，这正是 EXP-K04「与 CUB 差 0.7%」的机理解释。

**（b）gemv 领先幅度的机理线索（EXP-K01 §7 / EXP-K04 开放问题）**：
`gemv_v3` 与真 cuBLAS `gemv2T_kernel_val` 的 DRAM 吞吐几乎相同（95.47% vs 94.84%），
但 **achieved occupancy 相差悬殊：88.74% vs 63.75%**。
两者都贴在带宽墙上，v3 却能维持高得多的活跃 warp 数——这为"v3 快 34.1%"提供了
一个此前没有的机理候选（cuBLAS 侧 occupancy 不足），
可取代/补充原先"冷启动或时钟态"的猜测。**注意这是单点观察，不是结论**：
需要在 v3 与 cuBLAS 之间做 occupancy 受限因素（寄存器/smem/block 数）的逐项比对才能定论。
