...
> **⚠️ 后续更正（EXP-K09 §6.7）**：上面「两者不是同一个测量点（bench 扫多个 size，
> profiler 只钉住其中一个）」的说法**是错的**——`gemv/src/main.cu:35-36` 把形状写死为
> 4096×2048，bench 只有一个形状。真实原因是 **cache regime 不同**：
> 工作集 33.55 MB < 72 MB L2，bench 多轮迭代后矩阵常驻 L2（等效带宽 2633 GB/s，
> 超 DRAM 峰值 2.6 倍）；而 NCU 默认每次 replay 前 flush L2，测的是冷读的 HBM 区间。
> **34.1% 只在 L2 常驻区间成立**，HBM 区间两者持平。详见 EXP-K09 §5.8 / §6.7。
