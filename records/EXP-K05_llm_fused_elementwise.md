...
**H1 成立。** activation 的 v0→v1 实测 1.678x，与字节账预测的 5/3=1.667x 几乎精确吻合；fused-norm 1.509x 高于预测的 1.20x（v0 的两 kernel 之间隔了一整趟 1GB 写回， 第二个 kernel 的首次读全部 miss，实际字节账比静态计数更差）。自检条件通过：activation 的 v0(540.9)与 pytorch_eager(555.5)同速，说明未融合基线确实复刻了 eager 的执行方式，不是稻草人。
