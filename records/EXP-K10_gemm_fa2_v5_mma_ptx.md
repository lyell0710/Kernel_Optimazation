# EXP-K10 · gemm v5 = mma PTX + ldmatrix + smem padding（fa2 v5 待做）

> **一句话结论**：gemm v5 用 `mma.sync.m16n8k16` PTX + `ldmatrix` 重写 v4 微内核，**正确性 PASS、性能 135.1±2.1 TFLOPS = 真 cuBLAS 88.1%**（v4 = 132.8 = 85.6%，+1.7%）。**诚实负结果**：padding 消掉 ldmatrix 的 smem 读 bank conflict 后性能几乎不变，说明 EXP-K07 报告的「gemm v4 smem 冲突波前占 77.1%」**不是性能瓶颈**——「swizzle 能消除 gemm 的该瓶颈」这一推断对 gemm 被证伪，剩余 12% 差距在别处（多级流水 / tile 形状 / cp.async 调度）。

| 字段 | 值 |
|---|---|
| 日期 | 2026-08-30 |
| 环境 | 2×RTX 4090，CUDA 13.2（主力机），sm_89，driver 610.57.04 |
| 状态 | **gemm v5 完成；fa2 v5 待做** |
| 关联 | EXP-K02《CUDA Tensor Core GEMM 版本梯》§7；EXP-K07《NCU 计数器闭环》；LEDGER 红线「swizzle 能消除该瓶颈」「FA2 达到 sdpa/Triton 水平」 |
...
- **gemm 的「swizzle 能消除该瓶颈」推断被证伪（对 gemm）**：padding 已消 bank conflict，性能无显著变化。LEDGER 红线表该条需改写——对 gemm，剩余差距不在 smem，swizzle 不是解法。
- **fa2 v5 仍值得做**：FA2 与 gemm 不同，其 smem 紧张（90.75KB/99KB 上限），bank conflict 导致的额外 wavefront 更可能转化为 stall；且 FA2 的 softmax 需要 fragment→smem→标量段，ldmatrix 布局是解锁「寄存器驻留」的前提。判据不变：short_scoreboard + bank_conflicts 双降且端到端正收益。
- gemm v5 代码（`gemm_v5.cu`）正确性 PASS、已接入 bench，可保留；对外数字「gemm = cuBLAS 88.1%（CUDA 13.2）」的措辞更新（85.6% → 88.1% 需重过红线表）。
- 调试临时文件（`debug_v5.cu`、`dump_ldmatrix.cu`、`mma_test.cu`）不入库，删除。
