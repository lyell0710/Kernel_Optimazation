...
- [问题背景](#问题背景)
- [核心数据总览](#核心数据总览)
  - [cols=1024（完美对齐基准）](#cols1024完美对齐基准)
  - [cols=1500（非对齐场景）](#cols1500非对齐场景)
- [v0：并行化骨架（0.0262 ms）](#v0并行化骨架00262-ms)
- [v1：消除分支开销（0.0262 ms）](#v1消除分支开销00262-ms)
- [v2：修复 Shared Memory Bank Conflict（0.0257 ms）](#v2修复-shared-memory-bank-conflict00257-ms)
- [v3：向量化内存访问（0.0231 ms）](#v3向量化内存访问00231-ms)
- [v4：Warp-Level Reduction（0.0163 ms） ✨](#v4warp-level-reduction00163-ms)
- [v4.2：教学反例 ── 去掉 warp shuffle 会怎样？（0.0261 ms）](#v42教学反例--去掉-warp-shuffle-会怎样00261-ms)
- [v4.3：教学反例 ── main+tail 显式分离能让非对齐场景更快吗？](#v43教学反例--maintail-显式分离能让非对齐场景更快吗)
- [v4.4：教学反例 ── 故意制造 bank conflict 会怎样？（0.0237 ms）](#v44教学反例--故意制造-bank-conflict-会怎样00237-ms)
- [v4 系列的完整对比表](#v4-系列的完整对比表)
- [v4 vs cuBLAS：为什么 v4 快 26%？](#v4-vs-cublas为什么-v4-快-26)
  - [1. 向量化内存访问（float4）](#1-向量化内存访问float4)
  - [2. Warp-level shuffle](#2-warp-level-shuffle)
  - [3. Shared Memory 访问规整 + 参数特化](#3-shared-memory-访问规整--参数特化)
  - [综合：v4 vs cuBLAS 在不同 cols 上的优势](#综合v4-vs-cublas-在不同-cols-上的优势)
- [为什么 cuBLAS 不能直接抄 v4？](#为什么-cublas-不能直接抄-v4)
  - [约束 1：对齐方式未知](#约束-1对齐方式未知)
  - [约束 2：blockSize 必须运行时可调](#约束-2blocksize-必须运行时可调)
  - [约束 3：矩阵尺寸任意](#约束-3矩阵尺寸任意)
- [我学到的东西](#我学到的东西)
  - [1. 找瓶颈比盲目优化更重要](#1-找瓶颈比盲目优化更重要)
  - [2. 向量化暴露隐藏的瓶颈](#2-向量化暴露隐藏的瓶颈)
  - [3. Warp-level 原语的威力](#3-warp-level-原语的威力)
  - [4. 优化的链路依赖性（v4.2 / v4.4 的核心教训）](#4-优化的链路依赖性v42--v44-的核心教训)
  - [5. "非对齐"的真正含义（v4.3 的教训）](#5-非对齐的真正含义v43-的教训)
  - [6. 特化 vs 通用是架构权衡](#6-特化-vs-通用是架构权衡)
  - [7. 知道什么时候停止](#7-知道什么时候停止)
- [项目成果](#项目成果)
- [面试演讲稿（3 分钟）](#面试演讲稿3-分钟)
- [最终一句话总结](#最终一句话总结)
