...
> ⛔ **勘误(2026-08-24,红线级)**:下表原「cublas」行**并非 cuBLAS**——
> `softmax/src/softmax_cublas.cu` 系自写 warp 原语 kernel(cuBLAS 无 softmax API),
> 行名已改为 handwritten_ref;本文一切「vs cuBLAS」解读随之作废,
> 详见 records/EXP-K01 §5。数据本身(自写参照 kernel 的 NCU 计数)仍有效。
