...
**行宽为什么不是 64**。smem 有 32 个 bank，每 bank 宽 4B，一圈 128B。`Ssm` 若按 64 float 一行，行跨距 $64\equiv 0 \pmod{32}$，同一列的不同行落进同一个 bank；而 wmma 的 $16\times16$ store 恰恰是列向散布的，同一列的多行命中同一个 bank，退化成串行访问——按 4 B 访问计，同一 phase 的 32 个 lane 全撞，最坏 32 次（bank conflict 的判定作用域是 phase 不是 warp，phase 随访问宽度拆分，见 §3.3.1 末）。+4 float 的错位打散了这个模式。**为什么不是 +1(65)**：65 float 的行首偏移是 260B，不再是 16B 的倍数——float4 装载与 wmma 的对齐前提当场破坏。所以 68 =「打散 bank」与「保住 16B 对齐」两个约束下的最小值；`Psm` 是 half 粒度，16B = 8 half，同理取 72。
...
**bank conflict 的作用域是 phase 不是 warp**。本段与 §3.3 的「32 个 lane 全撞」都默认了「同一 warp 内 32 个 lane 共享一个作用域」，这不准确：宽于 4 字节的 shared memory 请求会被硬件拆成多个访存 phase，**冲突只在同一 phase 内判定**，而 phase 的大小由每线程访问宽度决定——4 B → 32 线程一 phase；8 B → 前 16 / 后 16 两个 phase；16 B（float4）→ 四个 phase、各 8 线程。同一 phase 内访问同一 bank 的**不同 slot** 才是冲突（同一 slot 是广播）。出处为课程课件 lesson21 作者对原视频表述的更正。对本段论证的影响：`Ssm`/`Psm` 的 wmma 访问宽度不公开（§3.3.1 已标账面推断），「32 次串行」是 4 B 访问下的最坏情形；若实际按 16 B 访问，phase 拆成 4×8，最坏路数从 31 降到 7。**这顺带给了「向量化访存降低 bank conflict 最坏路数」这条新直觉——向量化不只提带宽，还缩小了冲突的作用域。**
...
**误区 6：「测出来的带宽就是硬件的带宽」。** 判据只有一条：等效带宽对理论峰值。本仓 reduce 在 67.1 MB 数组上测得的等效带宽是 4090 HBM 理论峰值的 1.8 至 3.4 倍（EXP-K04《标准库基准补齐与两区间重测》§4.1）——**超过理论峰值只能说明数据没落 DRAM**，那个尺寸小于 72 MB 的 L2。同一份代码换成 1.07 GB 的数组，等效带宽立刻回到理论峰值的 94.5%（同口径 3 轮，`records/data/exp_k04_reduce_hbmbound_calfix_3rounds.csv`），而且版本之间的相对排序也变了。**一个性能数字的第一个属性不是大小，是「它测的是哪一层」。**
...
**Q16：附课里那条计时口径不对称，会不会推翻 reduce 的两区间结论？** 不会推翻，但会改变读法。`reduce_v7`/`reduce_v6` 在被计时的函数内部做 `cudaMalloc`/`cudaFree`(reduce_v7.cu：111-112、：125)，CUB 对照的 `temp_storage` 分配在计时外（reduce_cub.cu：23-28），而计时是包住单次调用的 event 对（cuda-reduce/src/main.cu：67-73）。量级上这处不对称与被讨论的差距同量级，所以本讲义当时的预判是：旧口径下 HBM 区间的「差 0.7%」更该读成「在测量分辨率内贴平」，旧口径下 L2 区间的 33.3% 里有多少来自算法本讲义无法判定。**方向是保守的**：消除不对称只会让自写侧变好或不变。检验方式明确（把分配提到计时外重测）——**这件事已经做完，两条预判都被同口径实测证实**(§8.3.4)：HBM 区间 v7 1.12659 对 CUB 1.12789 ms，在测量分辨率内贴平；L2 区间 CUB 快 **12.1%**（0.019808 对 0.022535 ms），旧口径的 33.3% 是它的 2.75 倍。数据见 `records/data/exp_k04_reduce_hbmbound_calfix_3rounds.csv` 与 `records/data/exp_k04_cuda_reduce_calfix_3rounds.csv`。
...
被测算子是 float 数组求和 $\sum x_i$，对照物是官方 **CUB `DeviceReduce::Sum`**（随 CUDA toolkit 分发的同算子官方实现）。分两个尺寸各跑 3 轮（RTX 4090，L2 = 72 MB，HBM 理论峰值 1008.1 GB/s；两侧计时口径已对齐——自写侧的中间数组分配与设备属性查询都提到计时外，与 CUB 的 `temp_storage` 同口径，见 §8.3.4；原始数据 `records/data/exp_k04_reduce_hbmbound_calfix_3rounds.csv` 与 `records/data/exp_k04_cuda_reduce_calfix_3rounds.csv`，过程见 EXP-K04 §4.1 与 EXP-K09 §5.17）：

| 区间 | 版本 | 时延（ms，mean±std） | 等效带宽 | 占理论峰值 |
|---|---|---|---|---|
| **HBM-bound**(N=1<<28,1.07 GB) | CUB | 1.12789±0.00041 | 952.0 GB/s | 94.4% |
| | **v7（自写最优）** | 1.12659±0.00033 | 953.1 GB/s | **94.5%** |
| | cuBLAS Sasum（异算子，仅作参照） | 1.14611±0.00025 | 936.9 GB/s | 92.9% |
| | v6 / v4 / v0 | 1.126 / 1.460 / 1.754 | 953.3 / 735.5 / 612.1 GB/s | 94.6 / 73.0 / 60.7% |
| **L2 常驻**(N=1<<24,67.1 MB) | CUB | 0.019808±0.000136 |（超理论峰值） | 不适用 |
| | v7 | 0.022535±0.000065 |（超理论峰值） | 不适用 |
| | cuBLAS Sasum | 0.037186±0.000310 |（超理论峰值） | 不适用 |

两个方向相反的结论：

- **真正 DRAM-bound 时，手写与官方库贴的是同一堵墙。** v7 与 CUB 分别达理论峰值的 94.5% 与 94.4%，**在测量分辨率内贴平**（1.12659 对 1.12789 ms，v7 名义上反快 0.1%，与轮间 std 同量级，不足以判定任何一方领先）。代码优劣的空间被同一条 DRAM 带宽线压到百分之一量级——这类算子的正确目标是**逼近峰值**，不是超越对手。
- **数据装进 L2 之后，厂商库的分尺寸调参才显出价值：CUB 快 12.1%**（0.019808 对 0.022535 ms）。瓶颈从 DRAM 带宽回到延迟隐藏、展开度、tile 尺寸与两阶段规约策略，CUB 按架构分派的 tuning 正是为此存在。自写 kernel 想追平，要做的是分尺寸调参，而不是再省一次访存。
...
- 三个版本（CUB / v7 / cuBLAS Sasum）在该尺寸下的等效带宽都远高于 HBM 理论峰值的 1008.1 GB/s。**等效带宽 1.8 至 3.4 倍于理论峰值，只能说明数据没落 DRAM。**
- 所以该区间**只报时延，不报带宽占比**——报了即错。

这条效度问题会污染两类结论。**第一类是跨机比较**：本仓早期在 4070 Laptop(32 MB L2)与 4090(72 MB L2)之间比较版本排序，同一个 67.1 MB 的尺寸在前者是 HBM-bound、在后者是 L2 常驻，**两台机器根本不在同一个区间**，排序变化的更可能解释是区间不同，而不是「roofline 迁移」。**第二类是「谁更快」的叙事**：同一份代码在两个区间给出「贴平」与「CUB 快 12.1%」两个完全不同的答案，只报其中一个都是片面的。
...
2. **X 必须是同一个算子**。reduce 曾用 `cublasSasum` 作对照，而 asum 算的是 $\sum|x_i|$，与被测的 $\sum x_i$ 语义不同。换成同算子的 CUB 之后才有了 §8.3.1 那张表；顺带一提，asum 在 HBM 区间也慢于 CUB（92.9% 对 94.4%），说明 BLAS 的规约路径本来就不是为纯求和优化的（EXP-K04 §5；两个百分比取 §8.3.1 表的同口径重测值）。
...
量级估计（**本讲义当时的分析，未实测；下面已由同口径重测闭合**）：旧口径下 HBM 区间 v7 是 1134.8 μs，0.7% 就是 8 μs；旧口径下 L2 区间 v7 是 29.7 μs，与 CUB 的差是 9.9 μs。**一对 4 KB 的 `cudaMalloc`/`cudaFree` 通常在微秒量级**，因此它在两个区间里都与被讨论的差距同量级。

**这不会推翻 §8.3.1 的两条结论，但会改变它们的读法。当时（未实测）写下的两条判断与后来同口径重测的结果并列如下，两条预判都命中**：

- **当时的判断**：HBM 区间的「差 0.7%」更准确的说法是「**在测量分辨率内两者贴平**」，而不是「CUB 略胜」。**已闭合**：同口径 3 轮实测 v7 1.12659±0.00033 ms 对 CUB 1.12789±0.00041 ms，v7 名义上反快 0.1%，与轮间 std 同量级——**贴平**成立，而且旧口径连「CUB 略胜」这个方向都站不住（`records/data/exp_k04_reduce_hbmbound_calfix_3rounds.csv`）。
- **当时的判断**：L2 区间的「CUB 快 33.3%」中有多少来自算法、多少来自这处口径不对称，**本讲义无法判定**。**已闭合**：同口径 3 轮实测 CUB 0.019808±0.000136 ms 对 v7 0.022535±0.000065 ms，**CUB 快 12.1%**；结论方向不变（CUB 仍然快），但旧口径的 33.3% 是真实幅度的 2.75 倍，虚高的那部分正是这处口径不对称（`records/data/exp_k04_cuda_reduce_calfix_3rounds.csv`）。

> **闭合过程（EXP-K09 §5.17）**：修正的是两件事，而本节当时只点出了 `cudaMalloc`/`cudaFree`；
> 更大的一项是 v6/v7 在计时区内调 `cudaGetDeviceProperties`（v1–v5 没有），
> 该调用在 driver 570 上单次可达毫秒级，曾使 v6/v7 一度测出 1.6–1.7 ms（慢 50 余倍）。
> 两项都提到计时外之后，两区间各重跑 3 轮，即上面两条与 §8.3.1 那张表的数据
> （`records/data/exp_k04_cuda_reduce_calfix_3rounds.csv`、
> `records/data/exp_k04_reduce_hbmbound_calfix_3rounds.csv`）。

**方向也预判对了**：消除不对称只会让自写 kernel 的数字变好或不变——实测 v7 在两个区间都变快（L2 0.029740 → 0.022535 ms，HBM 1.13483 → 1.12659 ms），旧口径对自写侧确实是**保守**的。检验方式当时也写明了：把 `d_partial` 的分配提到计时外（与 CUB 的 `temp_storage` 同口径）后重测两个区间。**这件事已经做完，§8.3.1 的表与两条结论用的就是修正后的数据。**
