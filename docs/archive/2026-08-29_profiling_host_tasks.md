...
> **superseded by `records/EXP-K09_post_vectorization_sector_ledger.md` + `LEDGER.md` @ 2026-08-29**
>
> 第二轮采集已完成（`6f320f2`：四算子共 23 份 `.ncu-rep` 重采，其中 `fused-norm` 六份为覆写、其余 17 份为新增，校验 `FAIL 0`，
> 结论入 EXP-K09《向量化修复后的扇区账复采》），本文件按其 §7 第 4 条退场，
> 从仓库根目录移到本处。状态与措辞的唯一权威回到 `LEDGER.md`。
>
> 保留原文而不是直接删除的理由只有一条：§5 的踩坑表是操作性资料，
> 且 `records/EXP-K07` §0、`records/EXP-K09` §0/§7 按旧路径引用本文件（史料不改）。
>
> **本文以下均为史料**：「本轮必做」「必须全部重采」「留着它超过一轮采集」这类现在时措辞
> 描述的是 2026-08-29 第二轮开工前的状态，现已不成立；§8.3 两个可选项的去向见 `LEDGER.md` 待办节。
